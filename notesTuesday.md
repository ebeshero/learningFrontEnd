...
In this example, Headline passes a prop called title. The `({title})` is a key in this:

```js
Headline({title: 'Hello', age: 12, show: true})
```

Look at **Spread Notation** in Day 1 notes: <https://github.com/oncomouse/dhsi2019/blob/master/day01.md>

***
I'm continuing by pasting in the Day 2 notes and adding my comments
***
...
###Notes
React components have rules: they must be capitalized, in opposition to HTML element names which are lower-cased.

Anything (any `const` variable) with a capitalized name is understood as a React component. 

Attributes for style in HTML: React doesn't let you put those in as strings: you have to pass them as objects in order to stop people from the Bad Practice of inserting CSS in their HTML. 

index.js in React imports some basic CSS, and some stuff React needs from something mysterious and magical called serviceWorker.

We've been rewriting the code in `src/App.js` and compiling / running to view on `localhost:3000` with `npm run start`


`<div id="root">...</div>` indicates to React that this is the place inside which React outputs its HTML

This is like Jekyll's includes. Or really, Gatsby does this in GitHub pages using React.

Lots of people build with React using Amazon sw3 or Heroku to be in a distributed cloud environment. 
...
[The file is also available on our GitHub](documents/javascript-snippets.json)
